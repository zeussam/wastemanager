...
<body class="antialiased bg-gray-100 dark-mode:bg-gray-900">
    ...
    <a href="{{route('admin.users.index')}}" style="background-color: #232628; color: white;" onmouseover="this.style.backgroundColor='#ef6339';" onmouseout="this.style.backgroundColor='#232628';" class="px-4 py-2 bg-green-500 hover:bg-green-700 rounded-md">Users Index</a>
</div>
    ...
    @if ($user->roles)
        @foreach ($user->roles as $user_role)
            <form method="POST" action="{{route('admin.users.roles.remove',[$user->id,$user_role->id])}}" onsubmit="return confirm('Are you sure?');">
                @csrf
                @method('DELETE')
                <button type="submit" style="background-color: #ef6339; color: white;" onmouseover="this.style.backgroundColor='#232628';" onmouseout="this.style.backgroundColor='#ef6339';" class="px-4 py-2 bg-green-500 hover:bg-green-700 rounded-md">{{$user_role->name}}</button>
            </form>
        @endforeach
    @endif
</div>
    ...
    @if ($user->permissions)
        @foreach ($user->permissions as $user_permission)
            <form method="POST" action="{{route('admin.users.permissions.revoke',[$user->id,$user_permission->id])}}" onsubmit="return confirm('Are you sure?');">
                @csrf
                @method('DELETE')
                <button type="submit" style="background-color: #ef6339; color: white;" onmouseover="this.style.backgroundColor='#232628';" onmouseout="this.style.backgroundColor='#ef6339';" class="px-4 py-2 bg-green-500 hover:bg-green-700 rounded-md">{{$user_permission->name}}</button>
            </form>
        @endforeach
    @endif
</div>
